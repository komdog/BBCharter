...
func change_animation(idx: int):
	...
	if Save.keyframes.has('loops') and Save.keyframes['loops'].size() > 0:
		if Global.song_pos > Save.keyframes['loops'][0]['timestamp']: run_loop()
...
func change_background(idx: int):
	...
	if bg.has('background_scale_multiplier'):
		if bg_type == 0:
			...
			$Panel/Static.scale = Vector2(bg['background_scale_multiplier'] * 2/3, bg['background_scale_multiplier'] * 2/3)
	else:
		if idx == 0:
			$Panel/Pattern.scale = Vector2(0.278, 0.281)
		elif bg_index == 1:
			$Panel/Static.scale = Vector2(0.667, 0.667)
